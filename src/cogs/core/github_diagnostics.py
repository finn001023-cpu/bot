...
from src.utils.github_manager import get_github_manager, GitHubDiagnostics, GitHubRequestQueue
...
class GitHubDiagnosticsCog(commands.Cog):
    ...
    async def _monitor_task(self):
        ...
        try:
            ...
            if rate_limit.get('rate', {}).get('remaining', 5000) < 100:
                print(f"[GitHub Alert] Low rate limit: {rate_limit['rate']['remaining']} remaining")
    ...
    async def github_diagnose(self, interaction: discord.Interaction, owner: str = None, repo: str = None):
        ...
        try:
            ...
            status_color = discord.Color.green() if results['api_status'] == 'operational' else discord.Color.red()
            embed.add_field(
                name="API Status",
                value=results['api_status'],
                inline=True
            )
            
            token_color = discord.Color.green() if results['token_valid'] else discord.Color.red()
            embed.add_field(
                name="Token Valid",
                value=str(results['token_valid']),
                inline=True
            )
            
            connectivity_color = discord.Color.green() if results['connectivity'] else discord.Color.red()
            embed.add_field(
                name="Connectivity",
                value=str(results['connectivity']),
                inline=True
            )
    ...
    async def github_status(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if 'rate' in rate_limit:
                ...
                color = discord.Color.green() if usage_percent < 80 else discord.Color.orange() if usage_percent < 95 else discord.Color.red()
                ...
                embed.add_field(
                    name="Status",
                    value="Normal" if remaining > 100 else "Low" if remaining > 10 else "Critical",
                    inline=True
                )
    ...
    async def github_fix(self, interaction: discord.Interaction, action: str, reset_token: bool = False):
        await interaction.response.defer(ephemeral=True)
        ...
        try:
            ...
            if action == "reinit":
                ...
                if reset_token:
                    await interaction.followup.send("Token reset requested - please restart bot with new token")
                    return
