...
class Achievements(commands.Cog):
    ...
    def unlock_achievement(
        ...
        if achievement_id not in achievements[user_key][guild_key]["unlocked"]:
            ...
            achievements[user_key][guild_key]["unlocked_at_" + achievement_id] = (
                datetime.now(TZ_OFFSET).isoformat()
            )
    ...
    def get_progress(self, user_id: int, guild_id: int = None) -> dict:
        ...
        if guild_id is None:
            ...
            return {"unlocked": len(unlocked), "total": total, "percentage": percentage}

        # 不考慮開發者成就在計算中
        regular_achievements = {
            k: v for k, v in ACHIEVEMENTS.items() if not v.get("developer_only", False)
        }
        total_regular = len(regular_achievements)

        percentage = (
            round((len(unlocked) / total_regular) * 100, 1) if total_regular > 0 else 0
        )
        return {
    ...
    async def achievements_command(
        ...
        try:
            ...
            unlocked_achievements = self.get_user_achievements(
            ...
            if unlocked_achievements:
                ...
                embed.add_field(
                    name="已解鎖成就", value="尚未解鎖任何成就", inline=False
                )
        ...
        try:
            ...
            for rarity in rarity_order:
                ...
                for ach_id, ach_data in achievements_by_rarity[rarity]:
                    ...
                    embed.add_field(
                        name=achievement_text, value=f"`{ach_id}`", inline=False
                    )
