...
class CNCookieBindModal(discord.ui.Modal, title="安全綁定國服 (米游社) 帳號"):
    """彈出式輸入視窗：安全輸入國服米游社 Cookie 各個欄位"""

    ltoken = discord.ui.TextInput(
        label="ltoken",
        style=discord.TextStyle.short,
        placeholder="請貼上 ltoken 的數值 (以 v2_CAES... 開頭的長字串)",
        required=True
    )
    ...
    cookie_token = discord.ui.TextInput(
    ...
    def __init__(self, service: GenshinService, cog: "GenshinCog"):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            # 驗證並綁定 Cookie
            accounts = await self.service.bind_account(interaction.user.id, cookie_str, "cn")
            ...
            for acc in accounts:
                game_name = self.cog._get_game_display_name(self.service._map_game_biz_to_str(acc["game_biz"]))
                embed.add_field(
                    name=f"🎮 {game_name}",
            ...
            embed.set_footer(text="預設已為您開啟每日自動簽到，如有異動可使用 /mhy toggle_autosignin 調整。")
            await interaction.followup.send(embed=embed, ephemeral=True)
            
        except Exception as e:
            await interaction.followup.send(
                f"❌ 綁定失敗：{str(e)}\n\n請確認您輸入的 `ltoken`、`ltuid` 是否正確且為米游社國服的數值。\n"
                f"若有疑問，請輸入 `/mhy tutorial` 查看獲取教學。",
                ephemeral=True
            )


class GlobalCookieBindModal(discord.ui.Modal, title="安全綁定國際服 (HoYoLAB) 帳號"):
    """彈出式輸入視窗：安全輸入國際服 HoYoLAB Cookie 各個欄位"""

    ltoken_v2 = discord.ui.TextInput(
        label="ltoken_v2",
        style=discord.TextStyle.short,
        placeholder="請貼上 ltoken_v2 的數值 (以 v2_CAES... 開頭的長字串)",
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        ltoken_v2_val = self.ltoken_v2.value.strip()
        ltuid_v2_val = self.ltuid_v2.value.strip()
        cookie_token_v2_val = self.cookie_token_v2.value.strip()

        # 構造標準的 Cookie 字串
        cookie_parts = [
            f"ltoken_v2={ltoken_v2_val};",
            f"ltuid_v2={ltuid_v2_val};"
        ]
        if cookie_token_v2_val:
            cookie_parts.append(f"cookie_token_v2={cookie_token_v2_val};")
            cookie_parts.append(f"account_id_v2={ltuid_v2_val};")

        cookie_str = " ".join(cookie_parts)

        try:
            # 驗證並綁定 Cookie
            accounts = await self.service.bind_account(interaction.user.id, cookie_str, "global")
            
            embed = discord.Embed(
                title="✅ 國際服 (HoYoLAB) 帳號綁定成功",
                description="您的 Cookie 已被安全地加密儲存於伺服器中，並已連結以下帳號：",
                color=discord.Color.from_rgb(46, 204, 113)
            )
            ...
            await interaction.followup.send(
                f"❌ 綁定失敗：{str(e)}\n\n請確認您輸入的 `ltoken_v2`、`ltuid_v2` 是否正確且為 HoYoLAB 國際服的數值。\n"
                f"若有疑問，請輸入 `/mhy tutorial` 查看獲取教學。",
                ephemeral=True
            )
...
class TutorialView(discord.ui.View):
    ...
    @discord.ui.button(label="🌐 國際服 (HoYoLAB) 教學", style=discord.ButtonStyle.primary, emoji="🌐")
    async def global_tutorial(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
        ...
        embed.add_field(
            name="1. 登入官網",
            value="請在電腦上使用 Chrome 或 Edge 等瀏覽器，開啟並登入 [HoYoLAB 官網](https://www.hoyolab.com/)",
            inline=False
        )
        embed.add_field(
            name="2. 開啟開發者工具",
        ...
        embed.add_field(
            name="4. 複製各個欄位數值 (Value)",
            value=(
                "在中間出現的 Cookies 表格中，依序找到以下變數，並**複製其 Value (值)**：\n"
                "• **`ltoken_v2`**：通常以 `v2_CAES...` 開頭的超長代碼 (必要)\n"
        ...
        embed.add_field(
            name="5. 在 Discord 中完成綁定",
            value="回到 Discord，輸入 `/mhy bind` 並選擇 **國際服 (HoYoLAB)**，隨後在彈出的對話框中貼入剛才複製的各個欄位數值並提交即可！",
            inline=False
        )
        embed.set_footer(text="⚠️ 提示：Cookie 相當於您的登入密碼，請勿向任何第三方透露或截圖發送！")
        await interaction.response.edit_message(embed=embed, view=self)

    @discord.ui.button(label="🇨🇳 國服 (米游社) 教學", style=discord.ButtonStyle.success, emoji="🇨🇳")
    async def cn_tutorial(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
            title="📖 國服 (米游社) Cookie 獲取教學",
            description="請跟隨以下步驟獲取您在米游社國服的登入憑證：",
            color=discord.Color.from_rgb(46, 204, 113)
        )
        embed.add_field(
            name="1. 登入官網",
            value="請在電腦上使用 Chrome 或 Edge 等瀏覽器，開啟並登入 [米游社官網](https://www.miyoushe.com/)",
            inline=False
        )
        embed.add_field(
            name="2. 開啟開發者工具",
        ...
        embed.add_field(
            ...
            value=(
                "在開發者工具頂部選單，點選「**Application (應用程式)**」頁籤（若沒看到，點擊 `>>` 展開選單）。\n"
                "接著在左側導覽列中展開「**Cookies**」，點擊 `https://www.miyoushe.com`。"
            ),
            inline=False
        )
        embed.add_field(
        ...
        embed.add_field(
            name="5. 在 Discord 中完成綁定",
            value="回到 Discord，輸入 `/mhy bind` 並選擇 **國服 (米游社)**，隨後在彈出的對話框中貼入剛才複製的各個欄位數值並提交即可！",
            inline=False
        )
        embed.set_footer(text="⚠️ 提示：Cookie 相當於您的登入密碼，請勿向任何第三方透露或截圖發送！")
        await interaction.response.edit_message(embed=embed, view=self)
...
class GenshinCog(commands.Cog):
    ...
    async def _init_character_names(self):
        try:
            ...
            await genshin.utility.update_characters_any(langs=["zh-cn", "zh-tw", "en-us"])
                    ...
                    try:
                        ...
                        if user:
                            ...
                            for res in user_results:
                                ...
                                lines.append(f"{status_emoji} **{game_name}** ({res['nickname']}): {res['message']}")
                            embed.description = "\n".join(lines)
                            embed.set_footer(text="可使用 /mhy toggle_autosignin 關閉此通知")
    ...
    @mhy_group.command(name="bind", description="安全綁定你的 HoYoLAB / 米游社 Cookie (會開啟視窗輸入)")
    ...
    async def mhy_bind(
        ...
        if region == "cn":
            modal = CNCookieBindModal(self.service, self)
        else:
            modal = GlobalCookieBindModal(self.service, self)
        await interaction.response.send_modal(modal)

    @mhy_group.command(name="tutorial", description="獲取米游社 / HoYoLAB Cookie 的教學指引")
    ...
    @mhy_group.command(name="unbind", description="解除綁定你的 HoYoLAB / 米游社 帳號並刪除 Cookie")
    async def mhy_unbind(self, interaction: discord.Interaction):
        ...
        if success:
            ...
            await interaction.followup.send("❌ 您尚未綁定任何 HoYoLAB/米游社 帳號。", ephemeral=True)
    ...
    async def mhy_status(self, interaction: discord.Interaction):
        ...
        region_display = "國服 (米游社)" if user_data.get("region") == "cn" else "國際服 (HoYoLAB)"
        auto_signin_display = "🟢 已開啟" if user_data.get("auto_sign_in", True) else "🔴 已關閉"
        ...
        if accounts:
            accounts_lines = []
            for acc in accounts:
                game_name = self._get_game_display_name(self.service._map_game_biz_to_str(acc["game_biz"]))
                accounts_lines.append(
                    f"• **{game_name}** - {acc['nickname']} (UID: `{acc['uid']}`) [Lv.{acc['level']} - {acc['server_name']}]"
                )
            embed.add_field(name="連結的遊戲帳號", value="\n".join(accounts_lines), inline=False)
        else:
            embed.add_field(name="連結的遊戲帳號", value="無連結的遊戲角色", inline=False)

        await interaction.followup.send(embed=embed, ephemeral=True)

    @mhy_group.command(name="toggle_autosignin", description="開啟或關閉每日自動簽到功能")
    ...
    async def mhy_checkin(self, interaction: discord.Interaction):
        ...
        try:
            results = await self.service.claim_daily_signin_for_user(interaction.user.id)
    ...
    @mhy_group.command(name="notes", description="查詢特定遊戲的實時便箋 (如原粹樹脂、開拓力、電量等)")
    @app_commands.describe(game="選擇要查詢的遊戲")
    @app_commands.choices(game=[
        app_commands.Choice(name="原神 (Genshin Impact)", value="genshin"),
        app_commands.Choice(name="崩壞：星穹鐵道 (Honkai: Star Rail)", value="starrail"),
        ...
        try:
            ...
            if game == "genshin":
                ...
                for exp in notes["expeditions"]:
                    ...
                    status_text = "✅ 已完成" if remained <= 0 else f"⏳ 剩餘 {self._format_seconds(remained)}"
                ...
                embed.add_field(
                    name="每日使命",
                    value=f"📅 **{notes['train_score']}**",
                    inline=True
                )
                
            embed.set_footer(text=f"UID: {notes['uid']} | 數據更新時間: {datetime.now(TZ_OFFSET).strftime('%Y-%m-%d %H:%M:%S')}")
            await interaction.followup.send(embed=embed)
            
        except Exception as e:
            await interaction.followup.send(f"❌ 查詢實時便箋時發生錯誤：{str(e)}", ephemeral=True)

    @mhy_group.command(name="redeem", description="為連結的遊戲帳號兌換禮包碼")
    @app_commands.describe(
        game="選擇禮包碼所屬遊戲",
        code="輸入兌換代碼 (Promo Code)"
    )
    @app_commands.choices(game=[
        app_commands.Choice(name="原神 (Genshin Impact)", value="genshin"),
        app_commands.Choice(name="崩壞：星穹鐵道 (Honkai: Star Rail)", value="starrail"),
    ...
    async def mhy_redeem(
        ...
        try:
            results = await self.service.redeem_code(interaction.user.id, game, code.strip())
    ...
    @mhy_group.command(name="stats", description="展示綁定角色基本統計數據（如成就數、活躍天數、箱子數）")
    @app_commands.describe(game="選擇要查詢的遊戲")
    @app_commands.choices(game=[
        app_commands.Choice(name="原神 (Genshin Impact)", value="genshin"),
        app_commands.Choice(name="崩壞：星穹鐵道 (Honkai: Star Rail)", value="starrail"),
    ...
    async def mhy_stats(
        ...
        try:
            ...
            embed.add_field(name="開拓等級" if game == "starrail" else "繩網等級" if game == "zzz" else "角色等級", value=f"Lv.{stats['level']}", inline=True)
            embed.add_field(name="活躍天數", value=f"{stats['days_active']} 天", inline=True)
            
            if "achievements" in stats:
                embed.add_field(name="達成成就", value=f"{stats['achievements']}", inline=True)
            if "characters" in stats:
                embed.add_field(name="角色/代理人數量", value=f"{stats['characters']}", inline=True)
            if "chests_opened" in stats:
                embed.add_field(name="開啟寶箱數", value=f"{stats['chests_opened']}", inline=True)
            if "teapot_level" in stats and stats["teapot_level"] > 0:
                embed.add_field(name="塵歌壺等級", value=f"Lv.{stats['teapot_level']}", inline=True)
            if "bangboo_obtained" in stats:
                embed.add_field(name="獲得邦布數", value=f"{stats['bangboo_obtained']}", inline=True)
                
            await interaction.followup.send(embed=embed)
            
        except Exception as e:
            await interaction.followup.send(f"❌ 獲取統計數據時發生錯誤：{str(e)}", ephemeral=True)
        ...
        try:
            ...
            if game == "genshin":
                if not abyss["is_unlock"]:
                    ...
                    embed.add_field(name="最深抵達層數", value=f"{abyss['max_floor']}", inline=True)
                    embed.add_field(name="獲得總星數", value=f"⭐ {abyss['total_stars']}", inline=True)
                    embed.add_field(name="戰鬥/勝利次數", value=f"{abyss['total_battles']} 次 / {abyss['total_wins']} 勝", inline=True)
                ...
                embed.add_field(name="式輿防衛戰進度", value=f"{abyss['shiyu_defense']}", inline=False)
                
            await interaction.followup.send(embed=embed)
            
        except Exception as e:
            await interaction.followup.send(f"❌ 獲取深淵數據時發生錯誤：{str(e)}", ephemeral=True)
