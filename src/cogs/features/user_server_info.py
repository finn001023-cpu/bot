...
class UserServerInfo(commands.Cog):
    ...
    async def user_info(
        ...
        try:
            ...
            if member and member.joined_at:
                ...
                embed.add_field(
                    name="加入伺服器時間", value="無法獲取或不在伺服器內", inline=False
                )
                ...
                try:
                    ...
                    if achievements_cog:
                        ...
                        achievements_cog.unlock_achievement(
                            user.id, interaction.guild_id, "info_explorer"
                        )
                ...
                try:
                    ...
                    if osu_cog:
                        ...
                        if bound_username:
                            ...
                            lines.append(
                                f"全球排名: {f'#{global_rank:,}' if isinstance(global_rank, int) else '未排名'}"
                            )
                            lines.append(
                                f"國家排名: {f'#{country_rank:,}' if isinstance(country_rank, int) else '未排名'}"
                            )
                            lines.append(
                                f"PP: {f'{pp:,.2f}' if isinstance(pp, (int, float)) else '未知'}"
                            )
                            lines.append(
                                f"準確度: {f'{acc:.2f}%' if isinstance(acc, (int, float)) else '未知'}"
                            )
                            lines.append(
    ...
    async def server_info(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if owner:
                embed.add_field(
                    name="擁有者", value=f"{owner.mention} ({owner.name})", inline=False
                )
            else:
