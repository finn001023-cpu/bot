        ...
        async with self._lock:
            if key not in self._sessions or self._sessions[key].closed:
                ...
                session = aiohttp.ClientSession(
                    ...
                    timeout=timeout,
                    headers=headers,
                    version=aiohttp.HttpVersion11 if not self.config.use_http2 else aiohttp.HttpVersion20
                )
    ...
    async def make_request(
        ...
        **kwargs
        ...
        try:
            ...
            self._active_requests[hostname] = max(0, self._active_requests.get(hostname, 0) - 1)
