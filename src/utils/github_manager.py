...
class GitHubRateLimitManager:
    ...
    async def wait_for_rate_limit(self, endpoint: str, headers: Dict) -> None:
        ...
        if rate_info['remaining'] <= 1:
            ...
            if wait_time > 0:
                print(f"[GitHub] Rate limit reached for {endpoint}, waiting {wait_time:.1f}s")
...
class GitHubAPIManager:
        ...
        for attempt in range(self.rate_manager.max_retries):
            try:
                async with session.request(method, url, **kwargs) as response:
                    await self.rate_manager.wait_for_rate_limit(endpoint, response.headers)
                    ...
                    if response.status == 200:
                        ...
                        print(f"[GitHub] {response.status} error, retry {attempt + 1}/{self.rate_manager.max_retries} in {delay}s")
                ...
                delay = self.rate_manager.get_retry_delay(attempt)
                print(f"[GitHub] Timeout error, retry {attempt + 1}/{self.rate_manager.max_retries} in {delay}s")
                await asyncio.sleep(delay)
                continue
            except aiohttp.ClientError as e:
                delay = self.rate_manager.get_retry_delay(attempt)
                print(f"[GitHub] Connection error, retry {attempt + 1}/{self.rate_manager.max_retries} in {delay}s: {e}")
                await asyncio.sleep(delay)
                continue
                ...
                if attempt == self.rate_manager.max_retries - 1:
                    raise e
                delay = self.rate_manager.get_retry_delay(attempt)
                print(f"[GitHub] Unexpected error, retry {attempt + 1}/{self.rate_manager.max_retries} in {delay}s: {e}")
                await asyncio.sleep(delay)
                continue
    ...
    async def get_pull_requests(self, owner: str, repo: str, per_page: int = 1) -> List[Dict]:
