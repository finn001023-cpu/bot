...
class DatabaseManager:
    ...
    async def store_metric(self, metric_name: str, value: float, metadata: Dict = None) -> bool:
        try:
            async with self.get_connection() as conn:
                timestamp = time.time()
    ...
    async def get_metrics(self, metric_name: str = None, limit: int = 100) -> List[Dict]:
        try:
            async with self.get_connection() as conn:
                if metric_name:
                    cursor = conn.execute("""
    ...
    async def log_audit(self, action: str, user_id: str = None, guild_id: str = None, details: Dict = None) -> bool:
        try:
            async with self.get_connection() as conn:
                timestamp = time.time()
